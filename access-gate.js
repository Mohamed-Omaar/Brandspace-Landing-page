(() => {
  ...
  style.textContent = `
    #brandspace-access-gate{position:fixed;inset:0;z-index:2147483647;display:grid;place-items:center;padding:22px;overflow:auto;background:radial-gradient(circle at 12% 8%,rgba(121,53,254,.28),transparent 34%),radial-gradient(circle at 90% 92%,rgba(255,221,21,.34),transparent 34%),radial-gradient(circle at 82% 18%,rgba(255,153,185,.22),transparent 28%),#f3f3f3;font-family:Inter,Cairo,"Segoe UI",Arial,sans-serif;color:#111114}
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (enteredHash === PASSWORD_HASH) {
      ...
      gate.animate([{opacity:1},{opacity:0}], {duration:220,easing:'ease'}).finished.then(() => gate.remove());
      document.documentElement.style.overflow = '';
      return;
    }
